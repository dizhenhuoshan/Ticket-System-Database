...
//                std::cerr << "pool is full" << std::endl;
		...
		void get_range(KeyType low, KeyType high, KeyType keys[], ValType arr[], int &len) {
			len = 0;
			...
			while(lowLeaf.keys[pos] < high) {
				if(keys != nullptr)     keys[len++] = lowLeaf.keys[pos];
				if(arr != nullptr)      arr[len++] = lowLeaf.vals[pos];
...
//                    #endif
		...
		KeyType back() {
			if(size() == 0) {
				...
				bm.get_tail(ret);
				cnt--;
				return ret.keys[ret.vals.size() - 1];
			}
		}
