...
namespace sjtu {
    ...
    struct station {
        ...
        void file_write(FILE *&fp, int seat_catalog_num) const {
            ...
            fwrite(&date_offset, sizeof(date_offset), 1, fp);
            // write prices.
            for (int i = 0; i < seat_catalog_num; ++i)
        ...
        void file_read(FILE *&fp, int seat_catalog_num) {
            ...
            fread(&date_offset, sizeof(date_offset), 1, fp);
            // write prices.
            for (int i = 0; i < seat_catalog_num; ++i)
    ...
    public:
        ...
        void read() {
            ...
            // set up date_offset.
            stations[0].date_offset = 0;
            for(int i = 1; i < station_num; ++i) {
                stations[i].date_offset = stations[i-1].date_offset;
                if(stations[i-1].arrive > stations[i].start || stations[i-1].start > stations[i].start)
                    ++stations[i].date_offset;
            }
        }
